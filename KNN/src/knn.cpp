...
#include "../../include/data_handler.h"
    ...
    int knn::predict()
    {
        ...
        {
            if(class_freq.find(neigbors->at(i)->get_label()) == class_freq.end())
            {
                class_freq[neigbors->at(i)->get_label()] = 1;
            } else
            {
                class_freq[neigbors->at(i)->get_label()]++;
            }
        }
...
int main()
{
    ...
    dh->count_classes();
    k *knearest = new knn();
    knearest->set_trainin_data(dh->get_training_data());
